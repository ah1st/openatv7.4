...
DEVICES=`find /dev/hd?`  # Modified to use /dev/hd?
...
# openATV 7.4 partition layout
sfdisk ${SELECTDEV} << EOF
label: dos
label-id: 0xc041c9d0
device: /dev/hdd
unit: sectors
/dev/hdd1 : start=        2048, size=      131072, type=c, bootable
/dev/hdd2 : start=      133120, size=      524288, type=82
/dev/hdd3 : start=     657408, size=   268435456, type=83
EOF
...
mkfs.vfat -n DREAM-BOOT /dev/hdd1
sleep 3
mount -t vfat /dev/hdd1 $BOOTNEW > /dev/null 2>&1
mkswap /dev/hdd2
swapon /dev/hdd2
yes | mkfs.ext4 -L ROOT /dev/hdd3
mount -t ext4 /dev/hdd3 $ROOTNEW > /dev/null 2>&1
...
echo "/dev/hdd1	/boot		vfat	ro				0 0" >> ${ROOTNEW}/etc/fstab
echo "/dev/hdd2	none		swap	sw				0 0" >> ${ROOTNEW}/etc/fstab
...
echo "/boot/vmlinux-3.2-${BOX}.gz root=/dev/hdd3 rootdelay=10 rootfstype=ext4 rw console=ttyS0,115200" >> ${BOOTNEW}/autoexec_${BOX}.bat

umount -f $BOOTNEW > /dev/null 2>&1
umount -f $ROOTNEW > /dev/null 2>&1
swapoff /dev/hdd2 > /dev/null 2>&1
